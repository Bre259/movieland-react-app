...
const TVSeriesCard = ({ series: { imdbID, Year, Poster, Title, Type } }) => {
  ...
  return (
    <>
      <div className="tv-series-card" key={imdbID} onClick={handleClick} style={{ cursor: 'pointer' }}>
      ...
      {showDetails && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content series-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {loading ? (
              <div className="loading">Loading series details...</div>
            ) : seriesDetails ? (
              <div className="series-details">
                <div className="series-details-header">
                  <img
                    src={seriesDetails.Poster !== "N/A" ? seriesDetails.Poster : "https://via.placeholder.com/300x450"}
                  ...
                  <div className="series-details-info">
                    ...
                    <p className="series-rating">⭐ {seriesDetails.imdbRating}/10</p>
                    <p className="series-runtime">{seriesDetails.Runtime}</p>
                    <p className="series-genre">{seriesDetails.Genre}</p>
                    <p className="series-director">Creator: {seriesDetails.Director}</p>
                    <p className="series-actors">Cast: {seriesDetails.Actors}</p>
                    
                    {/* TV Series specific info */}
                    {seriesDetails.totalSeasons && (
                      <p className="series-seasons">📺 {seriesDetails.totalSeasons} Season{seriesDetails.totalSeasons > 1 ? 's' : ''}</p>
                    )}
                ...
                <div className="series-details-awards">
                  <h3>Awards</h3>
                  <p>{seriesDetails.Awards || 'No awards information available'}</p>
                </div>
                  ...
                  <div className="series-stats">
                    {seriesDetails.totalSeasons && (
                      <div className="stat-item">
                        ...
                        <span className="stat-value">{seriesDetails.totalSeasons}</span>
                      </div>
                    )}
                    {seriesDetails.Runtime && (
                      <div className="stat-item">
                        <span className="stat-label">Episode Runtime:</span>
                        <span className="stat-value">{seriesDetails.Runtime}</span>
                      </div>
                    )}
                    {seriesDetails.Year && (
                      <div className="stat-item">
              ...
              <div className="error">Failed to load series details</div>
            )}
          </div>
        </div>
      )}
