...
const MovieCard = ({ movie: { imdbID, Year, Poster, Title, Type } }) => {
  ...
  const fetchMovieDetailsDirectly = async (movieId) => {
    try {
      const response = await fetch(
        `${OMDB_DIRECT_URL}?apikey=${OMDB_API_KEY}&i=${movieId}`
      );
  ...
  const fetchMovieDetails = async () => {
    ...
    try {
      ...
      console.log("Movie details loaded via Netlify Functions:", data);
    } catch (error) {
      console.warn(
        "Netlify Functions failed for movie details, trying direct OMDB API:",
        error.message
      );
      ...
      try {
        ...
        setMovieDetails(data);
        console.log("Movie details loaded via direct OMDB API:", data);
        console.info(
          "✅ Using direct OMDB API as fallback - movie details loaded successfully"
        );
      } catch (fallbackError) {
        console.error("Both APIs failed for movie details:", fallbackError);
